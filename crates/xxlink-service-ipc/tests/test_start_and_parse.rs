...
#[cfg(test)]
mod tests {
    use xxlink_service_ipc::{VERSION, run_ipc_server, stop_ipc_server};
    use xxlink_service_ipc::{connect, get_version};
    ...
    async fn test_reinstall_service_needed() {
        #[cfg(unix)]
        {
            ...
            assert!(
                !xxlink_service_ipc::is_ipc_path_exists(),
                "IPC path should not exist after stopping the server"
            );

            let ipc_path = Path::new(xxlink_service_ipc::IPC_PATH);
            ...
            assert!(
                xxlink_service_ipc::is_reinstall_service_needed().await,
