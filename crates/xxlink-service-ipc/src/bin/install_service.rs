...
fn resolve_service_group_name() -> String {
    ...
    if let Some(gid) = env_u32("XXLINK_SERVICE_GID")
...
fn main() -> Result<(), Error> {
    ...
    let service_binary_path = env::current_exe()
        .unwrap()
        .with_file_name("xxlink-service");

    if !service_binary_path.exists() {
        return Err(anyhow::anyhow!("xxlink-service binary not found"));
    }
    ...
    let target_binary_path = format!("{}/xxlink-service", macos_path);
    ...
    let _ = run_command(
        ...
        &[
            "enable",
            "system/com.xxlink.desktop.service",
        ],
        debug,
    );
    let _ = run_command(
    ...
    let _ = run_command(
        "launchctl",
        &["start", "com.xxlink.desktop.service"],
        debug,
    );
