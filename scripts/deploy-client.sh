...
INSTALLER=""
for candidate in \
  "target/x86_64-pc-windows-msvc/release/bundle/nsis/XXLink_${VERSION}_x64-setup.exe" \
  "target/x86_64-pc-windows-gnu/release/bundle/nsis/XXLink_${VERSION}_x64-setup.exe"; do
  if [[ -f "${candidate}" ]]; then
    INSTALLER="${candidate}"
    break
  fi
...
if [[ -z "${INSTALLER}" ]]; then
  INSTALLER="target/x86_64-pc-windows-msvc/release/bundle/nsis/XXLink_${VERSION}_x64-setup.exe"
fi
...
if [[ ! -f "${INSTALLER}" ]]; then
  ...
  echo "Run: corepack pnpm tauri build --target x86_64-pc-windows-msvc"
  exit 1
fi
