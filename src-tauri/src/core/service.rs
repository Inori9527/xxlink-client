...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = binary_path.with_file_name("xxlink-service-uninstall.exe");
...
fn install_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let install_path = binary_path.with_file_name("xxlink-service-install.exe");
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = tauri::utils::platform::current_exe()?.with_file_name("xxlink-service-uninstall");
...
fn install_service() -> Result<()> {
    ...
    let install_path = tauri::utils::platform::current_exe()?.with_file_name("xxlink-service-install");
...
fn uninstall_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let uninstall_path = binary_path.with_file_name("xxlink-service-uninstall");
    ...
    // xxlink_i18n::sync_locale(Config::verge().await.latest_arc().language.as_deref());

    let prompt = xxlink_i18n::t!("service.adminUninstallPrompt");
    let command =
...
fn install_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let install_path = binary_path.with_file_name("xxlink-service-install");
    ...
    let gid = tauri_plugin_xxlink_sysinfo::current_gid();
    let prompt = xxlink_i18n::t!("service.adminInstallPrompt");
    let command = format!(
        r#"do shell script "sudo XXLINK_SERVICE_GID={gid} '{install_shell}'" with administrator privileges with prompt "{prompt}""#
    );
...
pub(super) async fn start_with_existing_service(config_file: &PathBuf) -> Result<()> {
    ...
    let payload = xxlink_service_ipc::ClashConfig {
    ...
    let response = xxlink_service_ipc::start_clash(&payload)
        .await
        .context("无法连接到XXLink Service")?;
...
pub(super) async fn get_clash_logs_by_service() -> Result<Vec<CompactString>> {
    ...
    let response = xxlink_service_ipc::get_clash_logs()
        .await
        .context("无法连接到XXLink Service")?;
...
pub(super) async fn stop_core_by_service() -> Result<()> {
    ...
    let response = xxlink_service_ipc::stop_clash()
        .await
        .context("无法连接到XXLink Service")?;
...
pub async fn is_service_available() -> Result<()> {
    if let Err(e) = Path::metadata(xxlink_service_ipc::IPC_PATH.as_ref()) {
    ...
    xxlink_service_ipc::connect().await?;
    Ok(())
}
...
async fn wait_for_service_ipc(status: &mut ServiceManager, reason: &str) -> Result<()> {
    ...
    let result = (|| async {
        if Path::new(xxlink_service_ipc::IPC_PATH).exists() {
            xxlink_service_ipc::connect().await?;
            Ok(())
        } else {
...
pub fn is_service_ipc_path_exists() -> bool {
    Path::new(xxlink_service_ipc::IPC_PATH).exists()
}
...
impl ServiceManager {
    ...
    pub const fn config() -> xxlink_service_ipc::IpcConfig {
        xxlink_service_ipc::IpcConfig {
    ...
    pub async fn init(&mut self) -> Result<()> {
        if let Err(e) = xxlink_service_ipc::connect().await {
    ...
    pub async fn check_service_comprehensive(&self) -> ServiceStatus {
        if xxlink_service_ipc::is_reinstall_service_needed().await {
