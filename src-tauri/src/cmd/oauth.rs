...
pub async fn open_oauth_window(
    ...
    open::that(&final_url)
        .map_err(|e| smartstring::alias::String::from(format!("无法打开浏览器: {e}")))?;
    ...
    tauri::async_runtime::spawn(async move {
        let listener_handle = tokio::net::TcpListener::from_std(listener)
            ...
            Ok(Ok((code, error))) => {
                let _ = app_handle.emit(
                    ...
                    OAuthPayload {
                        code,
                        error,
                        redirect_uri: Some(redirect_uri_clone),
                    },
                );
            }
            Ok(Err(_)) | Err(_) => {
