...
pub struct IClashTemp(pub Mapping, pub(crate) bool);

impl IClashTemp {
    pub(crate) const fn source_read_failed(&self) -> bool {
        self.1
    }

    pub async fn new() -> Self {
        let path = match dirs::clash_path() {
            Ok(path) => path,
            Err(_) => {
                logging!(error, Type::Config, "Clash configuration path is unavailable");
        ...
        match help::read_mapping(&path).await {
            Ok(mut map) => {
                ...
                Self(Self::guard(map), false)
            }
            Err(_) => {
                logging!(error, Type::Config, "Clash configuration could not be loaded");
                let mut fallback = Self::template();
                fallback.1 = !matches!(tokio::fs::try_exists(&path).await, Ok(false));
                fallback
            }
        }
    }
    ...
    pub async fn save_config(&self) -> Result<()> {
        if self.1 {
            bail!("refusing to overwrite an unreadable Clash configuration");
        }
        help::save_yaml(&dirs::clash_path()?, &self.0, Some("# Generated by XXLink")).await
    }
...
fn test_clash_info() {
    fn get_case<T: Into<Value>, D: Into<Value>>(mp: T, ec: D) -> ClashInfo {
        ...
        IClashTemp(IClashTemp::guard(map), false).get_client_info()
    }
    ...
    assert_eq!(
        IClashTemp(IClashTemp::guard(Mapping::new()), false).get_client_info(),
