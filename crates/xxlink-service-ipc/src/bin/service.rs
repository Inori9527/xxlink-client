...
use xxlink_service_ipc::{run_ipc_server, stop_ipc_server};
...
fn main() -> Result<()> {
    init_logger();
    if service_dispatcher::start("xxlink_service", ffi_service_main).is_err() {
...
fn run_service() -> platform_lib::Result<()> {
    ...
    let status_handle = service_control_handler::register("xxlink_service", event_handler)?;
...
fn pid_file_path() -> PathBuf {
    #[cfg(unix)]
    {
        PathBuf::from("/tmp/xxlink/xxlink-service.pid")
    }
    #[cfg(windows)]
    {
        std::env::temp_dir().join("xxlink-service.pid")
    }
}
